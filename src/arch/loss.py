...
def get_labels(pts1, pts2, deformation):
    """
    pts1 = b, 1, 1, k, 3
    deformation = b, i, j, k, 3
    """
    device = pts1.device
    b, i, j, k, _ = deformation.shape
    ...
    """
    thresh = torch.tensor(8./float(i), device=device)

    # map landmarks in image 2 on image 1
    deformation = deformation.permute(0, 4, 1, 2, 3) # [B, 3, H, W, D]
    pts1_projected = F.grid_sample(deformation.to(device), pts2) #b, 3, 1, 1, k

    pts1_projected = pts1_projected.permute(0, 4, 2, 3, 1) #b, 1, 1, k, 3
    pts1_projected = pts1_projected.squeeze(dim=-2).squeeze(dim=-2)
    pts1 = pts1.squeeze(dim=1).squeeze(dim=1)

    # Add fake channel axes to different dims so that we get a 'cross' matrix
    pts1_projected = pts1_projected.unsqueeze(dim=1)
    pts1 = pts1.unsqueeze(dim=2)
...
def custom_loss(landmark_logits1, landmark_logits2, desc_pairs_score, desc_pairs_norm, gt1, gt2, match_target, k, device="cuda:0"):
    ...
    loss_dict['desc_loss_hinge_pos'] = pos_loss
    loss_dict['desc_loss_hinge_neg'] = neg_loss
