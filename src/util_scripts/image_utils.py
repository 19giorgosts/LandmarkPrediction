...
def extract_prostate_mask(path):
    print("Extracting prostate masks ...")
    ## extract the prostate mask and convert to .gipl (removing the .hdf)
    for k in os.listdir(path):
        if not k.startswith('dataset'):
            if os.path.isfile('{}/MR/mr_pv_res.hdf'.format(path+k)):
                print(k)
                # delineate prostate mask
                bashCommand = "java -jar {}/rasterDelineation.jar -gof {}/RTSTRUCT/voi.gof -rasterVals 1 -radthScanline 0 -structures {} -hdf {}/MR/mr_pv_res.hdf -background 0 -id 0 -single {}/prostate_mask.hdf"\
                    .format(javapath, path + k, "PTV_3625%ORGAN", path + k, path + k)
                process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
                output, error = process.communicate()

                # delineate body mask
                bashCommand = "java -jar {}/rasterDelineation.jar -gof {}/RTSTRUCT/voi.gof -rasterVals 1 -radthScanline 0 -structures {} -hdf {}/MR/mr_pv_res.hdf -background 0 -id 0 -single {}/body_mask.hdf" \
                    .format(javapath, path + k, "body%EXTERNAL", path + k, path + k)
                process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
                output, error = process.communicate()

                # converting.....
                if os.path.isfile('{}/prostate_mask.hdf'.format(path+k)) :
                    bashCommand = "java -jar {}/hdf2gipl.jar -infile {}/prostate_mask.hdf -outfile {}/prostate_mask.gipl".format(javapath, path +k,path +k)
                    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
                    output, error = process.communicate()
                    os.remove('{}/prostate_mask.hdf'.format(path+k))

                if os.path.isfile('{}/body_mask.hdf'.format(path+k)) :
                    bashCommand = "java -jar {}/hdf2gipl.jar -infile {}/body_mask.hdf -outfile {}/body_mask.gipl".format(javapath, path +k,path +k)
                    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
                    output, error = process.communicate()
                    os.remove('{}/body_mask.hdf'.format(path+k))


def convert_to_nii(path):
    # convert all files to .nii, renaming and moving to folders
    print("Converting and moving files ...")
    # en=0
    for j in (os.listdir(path)):
        if not j.startswith('dataset'):
            for item in os.listdir(path+'/'+j+'/'):
                if 'prostate_mask' in item:
                    print(j)
                    if not os.path.isdir(path+'/dataset/'+j):
                        os.mkdir(path+'/dataset/'+j)

                    ## converting prostate_mask.gipl to .nii
                    img = sitk.ReadImage(path+'/'+j+'/'+item)
                    sitk.WriteImage(img, path + '/dataset/' + j +'/' + '/'+item.split('.')[0]+'.nii')

                    ### converting mr_pre_res.hdf to .nii
                    bashCommand = "java -jar {}/hdf2gipl.jar -infile {}/MR/mr_pv_res.hdf -outfile {}/MR/mr_pv_res.gipl".format(javapath, path+'/'+j, path+'/'+j)
                    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
                    output, error = process.communicate()

                    mr_item='mr_pv_res.gipl'
                    img = sitk.ReadImage(path + '/' + j + '/MR/' + mr_item)

                    body_mask = 'body_mask.gipl'
                    img2 = sitk.ReadImage(path + '/' + j + '/' + body_mask)

                    res=img*img2
                    # sitk.WriteImage(res, path + '/' + j + '/' + mr_item.split('.')[0] + '.gipl')
                    sitk.WriteImage(res, path + '/dataset/' + '/' + j + '/' + mr_item.split('.')[0] + '.nii')
                    # incrementing the count
                    # en += 1

## generate the image pairs with a known transformation, generate the same transformation during training
def gryds_transform(args):
    path=args.data_dir
    for j in (os.listdir(path)):
        if j.startswith('dataset'):
            for item in os.listdir(path + '/' + j + '/'):
                print(item)
                img = sitk.ReadImage(path + '/' + j + '/'+ item+'/'+ 'mr_pv_res.nii')
                image=sitk.GetArrayFromImage(img)
                z,y,x,=image.shape
                batch_deformation_grid = create_batch_deformation_grid(shape=(1,1,z,y,x), device="cuda:0",dummy=args.dummy)
                images=torch.from_numpy(image)
                images=torch.unsqueeze(images,0)
                images = torch.unsqueeze(images, 0)
                # print(images.shape)
                images_hat = F.grid_sample(input=images.cuda(),
                                           grid=batch_deformation_grid,
                                           align_corners=True,
                                           mode="bilinear")
                # print(images_hat.shape)
                images_hat=torch.squeeze(images_hat, 0)
                images_hat=torch.squeeze(images_hat, 0)
                img2 = sitk.GetImageFromArray(images_hat.cpu())
                img2.SetSpacing((3,3,3))
                img2.SetOrigin(img.GetOrigin())
                sitk.WriteImage(img2, path + j + '/' + item + '/images_hat.nii')
                sitk.WriteImage(img2, path + j + '/' + item + '/images_hat.gipl')
    ...
    for j in (os.listdir(path)):
        if j.startswith('dataset'):
            for item in os.listdir(path + '/' + j + '/'):
                print(item)
                img = sitk.ReadImage(path + '/' + j + '/'+ item+'/'+ 'mr_pv_res.nii')
                # image1=sitk.GetArrayViewFromImage(img)
                # x,y,z = image1.shape
                # print(image1.shape)
                # deformation = generate_deformation_grid(image1)
                # print(np.max(deformation),np.min(deformation))
                dimension = 3
                deformation, transformation = affine_translate(img, dimension, 10, 0, 0) ## use in the dataloader in the future (I mean the transformation)
                arr=sitk.GetArrayFromImage(deformation)

                img2 = sitk.GetImageFromArray(arr)
                # img2.SetSpacing((3, 3, 3))
                # sitk.WriteImage(img2, path + j + '/' + item + '/deformation.gipl')
                #
                # image2 = F.grid_sample(torch.tensor(image1).view(1, 1, z,y,x), torch.tensor(deformation).view(1, z, y, x, 3).float(), mode="bilinear")
                # # print("sampled",image2.shape)
                # image2 = image2.squeeze(axis=0).squeeze(axis=0)
                # print(image2.shape)
                img2.SetSpacing((3,3,3))
                img2.SetOrigin(img.GetOrigin())
                sitk.WriteImage(img2, path + j + '/' + item + '/transformed.nii')
